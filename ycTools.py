...
def useRandom(text=''):
    ...
    while '{Z}' in text:
        text = text.replace('{Z}', random.choice(Z), 1)
    while '{n}' in text:
        text = text.replace('{n}', random.choice(n), 1)
    return text
def firstNameCnRandom():
    xing = [
        ...
        '熊', '纪', '舒', '屈', '项', '祝', '董', '梁']
    return xing[random.randint(0,len(xing)-1)]
def lastNameCnRandom():
    ming = [
    ...
    for x in range(random.randint(1, 2)):
        cache += ming[random.randint(0, len(ming)-1)]
    return cache
def proxyToProxies(proxy={'ip':'','port':0,'username':'','password':''}):
    proxies= None
    if proxy['ip'] != '':
        if proxy['username'] == '':
            proxies = {'http':'http://%s:%d' % (proxy['ip'], proxy['port']),
                    'https':'https://%s:%d' % (proxy['ip'], proxy['port'])}
        else:
            proxies = {'http':'http://%s:%s@%s:%d/' % (proxy['username'], proxy['password'], proxy['ip'], proxy['port']),
                   'https':'https://%s:%s@%s:%d/' % (proxy['username'], proxy['password'], proxy['ip'], proxy['port'])}
    return proxies
def proxyToProxies_AIOHTTP(proxy={'ip':'','port':0,'username':'','password':''}):
    proxies = None
    if proxy['ip'] != '':
        if proxy['username'] == '':
            proxies='http://%s:%d' % (proxy['ip'], proxy['port'])
        else:
            proxies='http://%s:%s@%s:%d' % (proxy['username'], proxy['password'], proxy['ip'], proxy['port'])
    return proxies
def maskPassword(str=''):
    text = ''
    for x in range(len(str)):
        if int(len(str) / 3) <= x and int(len(str) / 3 * 2) >= x:
            text += '*'
        else:
            text += str[x]
    return text
def getMiddleText(str='', behind='', under=''):
    try:
        return re.search(behind+"(.*?)"+under, str).group(1)
    except:
        return ''
def headersChange(text=''):
    ...
    return dic
def getIp():
    ret = requests.get('http://txt.go.sohu.com/ip/soip').text
...
def getChLastName():
    try:
        with open('lastName.txt', 'r') as f:
            lastName = f.readlines()
            return random.choice(lastName).strip()
    except:
        return ''
...
def userAgentisMobile(ua=''):
    ...
    if 'IEMobile' in ua:
        return True
    return False
def random_yueyuName():
    with open('yueyuName.txt', 'r', encoding='utf-8') as f:
        name = f.readlines()
    _name = name[random.randint(0, len(name) - 1)].split(',')[1].strip() + ' ' + \
            name[random.randint(0, len(name) - 1)].split(',')[1].strip() + ' ' + \
            name[random.randint(0, len(name) - 1)].split(',')[1].strip()
    return _name
def dictToRequestData(data={}):
    _ret = ''
    for name, value in data.items():
        _ret += '%s=%s&' % (name, value)
    return _ret[:-1]
class myConfig():
    def __init__(self, filename='setting.conf'):
        self.fileName = filename
        self.cf = configparser.ConfigParser()
        self.cf.read(self.fileName)
    def write(self, a='', b='', c=''):
        if a not in self.cf.sections():
            self.cf.add_section(a)
        self.cf.set(a, b, c)
        try:
            self.cf.write(open(self.fileName, 'w'))
        except Exception as e:
            print('Exception config write:%s' % e)
    def read(self, a='', b='', c=''):
        try:
            text = self.cf.get(a, b)
            return text
        except:
            return c
if __name__ == '__main__':
    print(getChFirstName())
    a={'a':{'b':'c'}}
    print(get_value_from_json(['a', 'b'], a))
    # a = mac()
    # print(a.readipconfig())
    # a.readinfo()
    # #print(readipconfig())
    print(getIp())
    # #headersChange()
    # print(getMiddleText('aaaaa1111bbbb', 'aaaaa', 'bbbb'))
    # print(maskPassword('YYC990726ccl'))
    # print(firstNameCnRandom())
    # print(lastNameCnRandom())
    # print(useRandom('{z}{z}{z}{Z}{Z}{Z}{n}{n}{n}{z}{z}{z}{Z}{Z}{Z}{n}'))
    # print(useRandom('{z}{Z}{n}{z}{Z}{n}{z}{Z}{n}{z}{Z}{n}{z}{Z}{n}'))
